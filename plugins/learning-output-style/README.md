...
**Note:** This plugin differs from the original unshipped Learning output style by also incorporating all functionality from the [explanatory-output-style plugin](https://github.com/opensins/opensin-code/tree/main/plugins/explanatory-output-style), providing both interactive learning and educational insights.
...
When enabled, this plugin automatically adds instructions at the start of each session that encourage OpenSIN to:
...
Instead of implementing everything automatically, OpenSIN will:
...
The plugin uses a SessionStart hook to inject additional context into every session. This context instructs OpenSIN to adopt an interactive teaching approach where you actively participate in writing key parts of the code.

## When OpenSIN requests contributions

OpenSIN will ask you to write code for:
...
## When OpenSIN won't request contributions

OpenSIN will implement directly:
...
**OpenSIN:** I've set up the authentication middleware. The session timeout behavior is a security vs. UX trade-off - should sessions auto-extend on activity, or have a hard timeout?
...
In addition to interactive learning, OpenSIN will provide educational insights about implementation choices using this format:
...
This SessionStart hook pattern is roughly equivalent to OPENSIN.md, but it is more flexible and allows for distribution through plugins.
...
- Update the plugin - create a local copy of this plugin to personalize it
  - Hint: Ask OpenSIN to read https://docs.opensin.com/en/docs/opensin-code/plugins.md and set it up for you!

## Philosophy

Learning by doing is more effective than passive observation. This plugin transforms your interaction with OpenSIN from "watch and learn" to "build and understand," ensuring you develop practical skills through hands-on coding of meaningful logic.
