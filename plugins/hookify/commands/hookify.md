...
```
{
  ...
  "prompt": "You are analyzing a OpenSIN Code conversation to find behaviors the user wants to prevent.
...
2. Corrections or reversions (user fixing OpenSIN's actions)
...
For each confirmed behavior, create a `.opensin/hookify.{rule-name}.local.md` file:
...
{Message to show OpenSIN when rule triggers}
```
...
**IMPORTANT**: Rule files must be created in the current working directory's `.opensin/` folder, NOT the plugin directory.

Use the current working directory (where OpenSIN Code was started) as the base path.

1. Check if `.opensin/` directory exists in current working directory
   - If not, create it first with: `mkdir -p .opensin`

2. Use Write tool to create each `.opensin/hookify.{name}.local.md` file
   - Use relative path from current working directory: `.opensin/hookify.{name}.local.md`
   - The path should resolve to the project's .opensin directory, not the plugin's

3. Show user what was created:
   ```
   Created 3 hookify rules:
   - .opensin/hookify.dangerous-rm.local.md
   - .opensin/hookify.console-log.local.md
   - .opensin/hookify.sensitive-files.local.md
...
4. Create `.opensin/hookify.dangerous-rm.local.md`:
   ```markdown
   ---
...
- **File location**: Create files in project's `.opensin/` directory (current working directory), NOT the plugin's .opensin/
...
2. Ensure `.opensin/` directory exists (create with mkdir if needed)
3. Use absolute path if needed: `{cwd}/.opensin/hookify.{name}.local.md`
4. Verify file was created with Glob or ls

**If rule doesn't trigger after creation:**
1. Verify file is in project `.opensin/` not plugin `.opensin/`
