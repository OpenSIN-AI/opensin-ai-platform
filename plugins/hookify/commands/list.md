...
1. Use Glob tool to find all hookify rule files:
   ```
   pattern: ".opensin/hookify.*.local.md"
   ```
...
**Status**: ✅ Active
**File**: .opensin/hookify.dangerous-rm.local.md
```
...
2. Or manually create `.opensin/hookify.my-rule.local.md` files
...
Check `${OPENSIN_PLUGIN_ROOT}/examples/` for example rule files.
```
