...
This analyzes your request and creates `.opensin/hookify.warn-rm.local.md`.

### 2. Test It Immediately

**No restart needed!** Rules take effect on the very next tool use.

Ask OpenSIN to run a command that should trigger the rule:
```
Run rm -rf /tmp/test
```
...
- **`stop`**: Triggers when OpenSIN wants to stop (for completion checks)
...
**This rule blocks the operation** - OpenSIN will not be allowed to execute these commands.
...
**This rule warns but allows** - OpenSIN sees the message but can still proceed.
...
**This blocks OpenSIN from stopping** if no test commands appear in the session transcript. Enable only when you want strict enforcement.
...
```bash
rm .opensin/hookify.my-rule.local.md
```
...
This plugin is part of the OpenSIN Code Marketplace. It should be auto-discovered when the marketplace is installed.
...
**Rule not triggering:**
1. Check rule file exists in `.opensin/` directory (in project root, not plugin directory)
