...
A comprehensive toolkit for developing OpenSIN Code plugins with expert guidance on hooks, MCP integration, plugin structure, and marketplace publishing.

## Overview

The plugin-dev toolkit provides seven specialized skills to help you build high-quality OpenSIN Code plugins:
...
4. **Plugin Settings** - Configuration patterns using .opensin/plugin-name.local.md files
...
7. **Testing** - Verify plugin works in OpenSIN Code
...
**Trigger phrases:** "create a hook", "add a PreToolUse hook", "validate tool use", "implement prompt-based hooks", "${OPENSIN_PLUGIN_ROOT}", "block dangerous commands"
...
- ${OPENSIN_PLUGIN_ROOT} for portable paths
...
- Environment variable expansion (${OPENSIN_PLUGIN_ROOT}, user vars)
...
- ${OPENSIN_PLUGIN_ROOT} usage throughout
...
**What it covers:**
- .opensin/plugin-name.local.md pattern for configuration
...
- AI-assisted agent generation using OpenSIN Code's proven prompt
...
- 3 reference docs: agent-creation-system-prompt (from OpenSIN Code), system-prompt-design (~4,000w), triggering-examples (~2,500w)
...
- Based on skill-creator methodology adapted for OpenSIN Code plugins
...
Install from opensin-code-marketplace:

```bash
/plugin install plugin-dev@opensin-code-marketplace
```
...
This keeps OpenSIN Code's context focused while providing deep knowledge when needed.
...
- Based on official OpenSIN Code documentation
...
✅ **Portability**
- Use ${OPENSIN_PLUGIN_ROOT} everywhere
- Relative paths only
...
This plugin is part of the opensin-code-marketplace. To contribute improvements:
...
Daisy Hollman (daisy@opensin.com)
