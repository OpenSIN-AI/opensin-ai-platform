...
Comprehensive guidance on creating OpenSIN Code slash commands, including file format, frontmatter options, dynamic arguments, and best practices.
...
- Plugin-specific features (${OPENSIN_PLUGIN_ROOT}, plugin patterns)
...
- **plugin-features-reference.md**: Plugin-specific command features
  - Plugin command discovery and organization
  - ${OPENSIN_PLUGIN_ROOT} environment variable usage
  - Plugin command patterns (configuration, template, multi-script)
...
OpenSIN Code activates this skill when users:
...
OpenSIN loads references and examples as needed based on task.
...
- **Project**: `.opensin/commands/` (shared with team)
- **Personal**: `~/.opensin/commands/` (your commands)
...
- ✓ Plugin command patterns (${OPENSIN_PLUGIN_ROOT}, discovery, organization)
...
6. Test examples work with current OpenSIN Code
