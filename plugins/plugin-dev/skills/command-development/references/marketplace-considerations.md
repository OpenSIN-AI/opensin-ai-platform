...
if [ ! -f ".opensin/command-initialized" ]; then
...
Checking for user config: .opensin/plugin-name.local.md

if [ -f ".opensin/plugin-name.local.md" ]; then
  # Parse YAML frontmatter for settings
  VERBOSE=$(grep "^verbose:" .opensin/plugin-name.local.md | cut -d: -f2 | tr -d ' ')
  COLOR=$(grep "^color:" .opensin/plugin-name.local.md | cut -d: -f2 | tr -d ' ')
  MAX_RESULTS=$(grep "^max_results:" .opensin/plugin-name.local.md | cut -d: -f2 | tr -d ' ')

  echo "✓ Using user configuration"
else
  echo "Using default configuration"
  echo "Create .opensin/plugin-name.local.md to customize"
fi
...
Or set in .opensin/plugin-name.local.md:
\`\`\`yaml
---
format: yaml
output: custom.txt
...
if [ -f ".opensin/operation-completed.flag" ]; then
  ℹ️  Operation already completed

  Completed at: $(cat .opensin/operation-completed.flag)

  To re-run:
  1. Remove flag: rm .opensin/operation-completed.flag
  2. Run command again
...
Marking complete...
echo "$(date)" > .opensin/operation-completed.flag
```
