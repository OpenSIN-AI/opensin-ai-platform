...
This reference covers features and patterns specific to commands bundled in OpenSIN Code plugins.

## Table of Contents

- [Plugin Command Discovery](#plugin-command-discovery)
- [OPENSIN_PLUGIN_ROOT Environment Variable](#opensin_plugin_root-environment-variable)
...
OpenSIN Code automatically discovers commands in plugins using the following locations:
...
## OPENSIN_PLUGIN_ROOT Environment Variable

### Purpose

`${OPENSIN_PLUGIN_ROOT}` is a special environment variable available in plugin commands that resolves to the absolute path of the plugin directory.
...
Run analysis: !`node ${OPENSIN_PLUGIN_ROOT}/scripts/analyze.js`

Read template: @${OPENSIN_PLUGIN_ROOT}/templates/report.md
```
...
Lint results: !`node ${OPENSIN_PLUGIN_ROOT}/bin/lint.js $1`
...
Configuration: @${OPENSIN_PLUGIN_ROOT}/config/deploy-config.json
...
Use this template: @${OPENSIN_PLUGIN_ROOT}/templates/api-report.md
...
Step 1 - Prepare: !`bash ${OPENSIN_PLUGIN_ROOT}/scripts/prepare.sh $1`
Step 2 - Config: @${OPENSIN_PLUGIN_ROOT}/config/$1.json
Step 3 - Execute: !`${OPENSIN_PLUGIN_ROOT}/bin/execute $1`
...
1. **Always use for plugin-internal paths:**
   ```markdown
   # Good
   @${OPENSIN_PLUGIN_ROOT}/templates/foo.md
...
2. **Validate file existence:**
   ...
   !`test -f ${OPENSIN_PLUGIN_ROOT}/config.json && echo "exists" || echo "missing"`

   If config exists, load it: @${OPENSIN_PLUGIN_ROOT}/config.json
...
4. **Combine with arguments:**
   ```markdown
   Run: !`${OPENSIN_PLUGIN_ROOT}/bin/process.sh $1 $2`
   ```
...
- Verify syntax is exact: `${OPENSIN_PLUGIN_ROOT}`
...
Load configuration: @${OPENSIN_PLUGIN_ROOT}/deploy-config.json
...
Template: @${OPENSIN_PLUGIN_ROOT}/templates/component-docs.md
...
Validate: !`bash ${OPENSIN_PLUGIN_ROOT}/scripts/validate.sh`
...
Environment config: @${OPENSIN_PLUGIN_ROOT}/config/$1.json
...
Cache directory: ${OPENSIN_PLUGIN_ROOT}/cache/

Analyze @$1 and save results to cache:
!`mkdir -p ${OPENSIN_PLUGIN_ROOT}/cache && date > ${OPENSIN_PLUGIN_ROOT}/cache/last-run.txt`
...
- OpenSIN will automatically use Task tool to launch agent
...
- Mention skill by name to hint OpenSIN should invoke it
...
- Config exists: !`test -f ${OPENSIN_PLUGIN_ROOT}/config.json && echo "✓" || echo "✗"`
- Scripts exist: !`test -d ${OPENSIN_PLUGIN_ROOT}/scripts && echo "✓" || echo "✗"`
- Tools available: !`test -x ${OPENSIN_PLUGIN_ROOT}/bin/analyze && echo "✓" || echo "✗"`
...
Try processing: !`node ${OPENSIN_PLUGIN_ROOT}/scripts/process.js $1 2>&1 || echo "ERROR: $?"`
...
1. **Use ${OPENSIN_PLUGIN_ROOT} for all plugin-internal paths**
