...
head -n 20 .opensin/commands/my-command.md | grep -A 10 "^---"

# Check for closing frontmatter marker
head -n 20 .opensin/commands/my-command.md | grep -c "^---" # Should be 2

# Verify file has .md extension
ls .opensin/commands/*.md

# Check file is in correct location
test -f .opensin/commands/my-command.md && echo "Found" || echo "Missing"
```
...
# 5. Check debug logs
tail -f ~/.opensin/debug-logs/latest
...
cat > .opensin/commands/test-bash.md << 'EOF'
---
...
cat > .opensin/commands/test-forbidden.md << 'EOF'
---
...
# Command: .opensin/commands/risky-operation.md
...
COMMANDS_CHANGED=$(git diff --cached --name-only | grep "\.opensin/commands/.*\.md")
  ...
  test:
    ...
    steps:
      ...
      - name: Validate command structure
        run: |
          for cmd in .opensin/commands/*.md; do
            echo "Testing: $cmd"
      ...
      - name: Check for TODOs
        run: |
          if grep -r "TODO" .opensin/commands/; then
...
```bash
# Monitor OpenSIN Code during command execution
# In terminal 1:
opensin --debug

# In terminal 2:
watch -n 1 'ps aux | grep opensin'
...
```bash
# Check file location
ls -la .opensin/commands/my-command.md

# Check permissions
chmod 644 .opensin/commands/my-command.md

# Check syntax
head -n 20 .opensin/commands/my-command.md
...
```bash
# Verify syntax
grep '\$1' .opensin/commands/my-command.md
grep '\$ARGUMENTS' .opensin/commands/my-command.md

# Test with simple command first
echo "Test: \$1 and \$2" > .opensin/commands/test-args.md
```
...
```bash
# Check allowed-tools
grep "allowed-tools" .opensin/commands/my-command.md

# Verify command syntax
grep '!\`' .opensin/commands/my-command.md
