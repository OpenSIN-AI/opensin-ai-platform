...
Deployment state saved to `.opensin/deployment-state.local.md`:
...
Reading deployment state from `.opensin/deployment-state.local.md`...
...
```markdown
.opensin/plugin-name-workflow.local.md:
...
Reading workflow state from .opensin/plugin-name-workflow.local.md...

Current stage: @.opensin/plugin-name-workflow.local.md
...
State file: @.opensin/plugin-name-workflow.local.md
...
Creating: .opensin/feature-complete.flag
...
if [ -f .opensin/feature-complete.flag ]; then
...
if [ -f .opensin/deployment.lock ]; then
...
!`validate.sh`
echo "checkpoint:validation" >> .opensin/deployment-checkpoints.log

## Checkpoint 2: Build
!`build.sh`
echo "checkpoint:build" >> .opensin/deployment-checkpoints.log

## Checkpoint 3: Deploy
!`deploy.sh`
echo "checkpoint:deploy" >> .opensin/deployment-checkpoints.log
...
Written to .opensin/deployment-state.local.md
...
Reading state: @.opensin/deployment-state.local.md
...
rm .opensin/deployment-state.local.md
