...
check_script() {
  ...
  if grep -E '^[^#]*/home/|^[^#]*/usr/|^[^#]*/opt/' "$script" | grep -q .; then
    ...
    echo "   Use \$OPENSIN_PROJECT_DIR or \$OPENSIN_PLUGIN_ROOT"
    ((warnings++))
  fi

  # Check 8: Uses OPENSIN_PLUGIN_ROOT
  if ! grep -q "OPENSIN_PLUGIN_ROOT\|OPENSIN_PROJECT_DIR" "$script"; then
    echo "💡 Tip: Use \$OPENSIN_PLUGIN_ROOT for plugin-relative paths"
  fi
