...
This reference provides common, proven patterns for implementing OpenSIN Code hooks. Use these patterns as starting points for typical hook use cases.
...
if [ -f "package.json" ]; then
  ...
  echo "export PROJECT_TYPE=rust" >> "$OPENSIN_ENV_FILE"
fi
```
  ...
  "Notification": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "bash ${OPENSIN_PLUGIN_ROOT}/scripts/log-notification.sh"
        }
      ]
    }
  ]
  ...
  "PostToolUse": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "bash ${OPENSIN_PLUGIN_ROOT}/scripts/check-quality.sh"
        }
      ]
    }
  ]
...
FLAG_FILE="$OPENSIN_PROJECT_DIR/.enable-security-scan"
...
**Note:** Must restart OpenSIN Code after creating/removing flag files for hooks to recognize changes.
...
```bash
#!/bin/bash
CONFIG_FILE="$OPENSIN_PROJECT_DIR/.opensin/my-plugin.local.json"
...
**Configuration file (.opensin/my-plugin.local.json):**
```json
{
  "strictMode": true,
  "maxFileSize": 500000,
