  ...
  "PreToolUse": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "bash ${OPENSIN_PLUGIN_ROOT}/scripts/quick-check.sh",
          "timeout": 5
        },
        {
          "type": "prompt",
...
if [ -f ".opensin-hooks-config.json" ]; then
  strict_mode=$(jq -r '.strict_mode' .opensin-hooks-config.json)
...
**Example .opensin-hooks-config.json:**
```json
{
  "strict_mode": true,
...
# Append to audit log
echo "$timestamp | $USER | $tool_name | $input" >> ~/.opensin/audit.log
...
export OPENSIN_PROJECT_DIR="/tmp/test-project"
export OPENSIN_PLUGIN_ROOT="$(pwd)"
mkdir -p "$OPENSIN_PROJECT_DIR"
