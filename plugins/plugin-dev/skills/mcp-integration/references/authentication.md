...
Complete guide to authentication methods for MCP servers in OpenSIN Code plugins.
...
OpenSIN Code automatically handles the complete OAuth 2.0 flow for SSE and HTTP servers:

1. User attempts to use MCP tool
2. OpenSIN Code detects authentication needed
3. Opens browser for OAuth consent
4. User authorizes in browser
5. Tokens stored securely by OpenSIN Code
...
No additional auth configuration needed! OpenSIN Code handles everything.
...
Tokens are stored securely by OpenSIN Code:
...
```json
{
  "api": {
    ...
    "headersHelper": "${OPENSIN_PLUGIN_ROOT}/scripts/get-headers.sh"
  }
}
```
...
{
  "secure-api": {
    "command": "${OPENSIN_PLUGIN_ROOT}/servers/mtls-wrapper",
...
```json
{
  "headersHelper": "${OPENSIN_PLUGIN_ROOT}/scripts/generate-jwt.sh"
}
```
