...
Complete reference for all MCP server types supported in OpenSIN Code plugins.
...
{
  "my-server": {
    "command": "${OPENSIN_PLUGIN_ROOT}/servers/custom-server",
...
1. **Startup**: OpenSIN Code spawns process with `command` and `args`
2. **Communication**: JSON-RPC messages via stdin/stdout
3. **Lifecycle**: Process runs for entire OpenSIN Code session
4. **Shutdown**: Process terminated when OpenSIN Code exits
...
{
  "custom": {
    "command": "${OPENSIN_PLUGIN_ROOT}/servers/my-server.js",
    "args": ["--verbose"]
  }
}
```
...
1. **Use absolute paths or ${OPENSIN_PLUGIN_ROOT}**
...
OpenSIN Code handles OAuth flow:
