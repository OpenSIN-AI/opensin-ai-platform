...
Complete guide to using MCP tools effectively in OpenSIN Code plugin commands and agents.

## Overview

Once an MCP server is configured, its tools become available with the prefix `mcp__plugin_<plugin-name>_<server-name>__<tool-name>`. Use these tools in commands and agents just like built-in OpenSIN Code tools.
...
- Can use any tool OpenSIN determines is necessary
...
OpenSIN automatically structures tool calls based on schema:

```typescript
// OpenSIN generates this internally
{
...
```markdown
Steps:
1. Make parallel calls (OpenSIN handles this automatically):
...
2. **Install plugin locally** in `.opensin-plugin/`
3. **Verify tools available** with `/mcp`
4. **Test command** that uses tools
5. **Check debug output**: `opensin --debug`
