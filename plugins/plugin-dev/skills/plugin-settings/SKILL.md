...
description: This skill should be used when the user asks about "plugin settings", "store plugin configuration", "user-configurable plugin", ".local.md files", "plugin state files", "read YAML frontmatter", "per-project plugin settings", or wants to make plugin behavior configurable. Documents the .opensin/plugin-name.local.md pattern for storing plugin-specific configuration with YAML frontmatter and markdown content.
version: 0.1.0
---

# Plugin Settings Pattern for OpenSIN Code Plugins

## Overview

Plugins can store user-configurable settings and state in `.opensin/plugin-name.local.md` files within the project directory. This pattern uses YAML frontmatter for structured configuration and markdown content for prompts or additional context.

**Key characteristics:**
- File location: `.opensin/plugin-name.local.md` in project root
...
- Prompts to feed back to OpenSIN
...
STATE_FILE=".opensin/my-plugin.local.md"
...
Steps:
1. Check if settings exist at `.opensin/my-plugin.local.md`
...
Check for plugin settings at `.opensin/my-plugin.local.md`.
...
```bash
#!/bin/bash
STATE_FILE=".opensin/security-scan.local.md"
...
**.opensin/multi-agent-swarm.local.md:**
```markdown
---
...
2. Create `.opensin/my-plugin.local.md` with YAML frontmatter
3. Set appropriate values based on user input
4. Inform user that settings are saved
5. Remind user to restart OpenSIN Code for hooks to recognize changes
```
...
Create `.opensin/my-plugin.local.md` in your project:
...
After creating or editing, restart OpenSIN Code for changes to take effect.
```
...
✅ **DO:**
- Use `.opensin/plugin-name.local.md` format
- Match plugin name exactly
- Use `.local.md` suffix for user-local files

❌ **DON'T:**
- Use different directory (not `.opensin/`)
...
**Important:** Settings changes require OpenSIN Code restart.
...
After editing `.opensin/my-plugin.local.md`:
1. Save the file
2. Exit OpenSIN Code
...
```bash
if [[ ! -f ".opensin/my-plugin.local.md" ]]; then
...
3. Add gitignore entry for `.opensin/*.local.md`
...
7. Remind users that changes require OpenSIN Code restart
