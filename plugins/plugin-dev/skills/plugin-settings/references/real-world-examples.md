...
Detailed analysis of how production plugins use the `.opensin/plugin-name.local.md` pattern.
...
SWARM_STATE_FILE=".opensin/multi-agent-swarm.local.md"
...
```bash
cat > "$WORKTREE_PATH/.opensin/multi-agent-swarm.local.md" <<EOF
---
...
sed "s/^pr_number: .*/pr_number: $PR_NUM/" \
  ".opensin/multi-agent-swarm.local.md" > temp.md
mv temp.md ".opensin/multi-agent-swarm.local.md"
```
...
Document any changes needed in OPENSIN.md.
```
...
**Purpose:** Prevent session exit and loop OpenSIN's output back as input
...
# Create state file
cat > ".opensin/ralph-loop.local.md" <<EOF
---
iteration: 1
...
echo "Ralph loop initialized: .opensin/ralph-loop.local.md"
```
...
|---------|-------------------|--------------|
| **File** | `.opensin/multi-agent-swarm.local.md` | `.opensin/ralph-loop.local.md` |
...
```bash
# BAD
FILE="/Users/alice/.opensin/my-plugin.local.md"

# GOOD
FILE=".opensin/my-plugin.local.md"
```
...
The `.opensin/plugin-name.local.md` pattern provides:
