...
**Required path**: `.opensin-plugin/plugin.json`

The manifest MUST be in the `.opensin-plugin/` directory at the plugin root. OpenSIN Code will not recognize plugins without this file in the correct location.
...
- Plugin identification in OpenSIN Code
...
```json
{
  "mcpServers": {
    "github": {
      "command": "node",
      "args": ["${OPENSIN_PLUGIN_ROOT}/servers/github-mcp.js"],
      "env": {
...
When OpenSIN Code loads components:
...
OpenSIN Code validates the manifest on plugin load:
