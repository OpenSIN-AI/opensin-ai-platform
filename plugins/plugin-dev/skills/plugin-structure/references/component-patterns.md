...
When OpenSIN Code starts:

1. **Scan enabled plugins**: Read `.opensin-plugin/plugin.json` for each
2. **Discover components**: Look in default and custom paths
3. **Parse definitions**: Read YAML frontmatter and configurations
4. **Register components**: Make available to OpenSIN Code
5. **Initialize**: Start MCP servers, register hooks

**Timing**: Component registration happens during OpenSIN Code initialization, not continuously.

### Activation Phase

When components are used:

**Commands**: User types slash command → OpenSIN Code looks up → Executes
**Agents**: Task arrives → OpenSIN Code evaluates capabilities → Selects agent
**Skills**: Task context matches description → OpenSIN Code loads skill
**Hooks**: Event occurs → OpenSIN Code calls matching hooks
...
**Note**: OpenSIN Code doesn't support nested command discovery automatically. Use custom paths:
...
**Note**: Use build script to combine files, OpenSIN Code doesn't support file references.
...
```bash
#!/bin/bash
source "${OPENSIN_PLUGIN_ROOT}/lib/test-utils.sh"
run_tests
```
