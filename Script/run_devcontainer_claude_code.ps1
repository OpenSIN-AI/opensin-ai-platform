...
.EXAMPLE
    .\Script\run_devcontainer_opensin_code.ps1 -Backend docker
    Uses Docker as the container backend.

.EXAMPLE
    .\Script\run_devcontainer_opensin_code.ps1 -Backend podman
...
.NOTES
    ...
    ├── .devcontainer/
    └── Script/
        └── run_devcontainer_opensin_code.ps1
#>
...
if ($Backend -eq 'podman') {
    ...
    Write-Host "Initializing Podman machine 'opensinVM'..."
    try {
        & podman machine init opensinVM
        Write-Host "Podman machine 'opensinVM' initialized or already exists."
    } catch {
    ...
    Write-Host "Starting Podman machine 'opensinVM'..."
    try {
        & podman machine start opensinVM -q
    ...
    Write-Host "Setting default Podman connection to 'opensinVM'..."
    try {
        & podman system connection default opensinVM
...
Write-Host "Executing 'opensin' command and then starting zsh session inside container $($containerId)..."
try {
    & $Backend exec -it $containerId zsh -c 'opensin; exec zsh'
    Write-Host "Interactive session ended."
} catch {
    $displayCommand = "$Backend exec -it $containerId zsh -c 'opensin; exec zsh'"
