...
4. First, check if this issue is actually about OpenSIN Code (the CLI/IDE tool). Issues about the OpenSIN API, opensin.ai, the OpenSIN app, OpenSIN billing, or other OpenSIN products should be labeled `invalid`. If invalid, apply only that label and stop.
...
6. Evaluate lifecycle labels:
   - `needs-repro` (bugs only, 7 days): Bug reports without clear steps to reproduce. A good repro has specific, followable steps that someone else could use to see the same issue.
     ...
     For model behavior issues (e.g. "OpenSIN does X when it should do Y"), don't require traditional repro steps — examples and patterns are sufficient.
