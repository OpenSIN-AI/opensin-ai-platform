...
- Fixed Homebrew install update prompts to use the cask's release channel (`opensin-code` → stable, `opensin-code@latest` → latest)
...
- Fixed plugin MCP servers stuck "connecting" on session start when they duplicate a opensin.ai connector that is unauthenticated
...
- Added MCP tool result persistence override via `_meta["opensin/maxResultSizeChars"]` annotation (up to 500K), allowing larger results like DB schemas to pass through without truncation
...
- Added support for multi-line prompts in `opensin-cli://open?q=` deep links (encoded newlines `%0A` no longer rejected)
...
- Improved `/opensin-api` skill guidance for agent design patterns including tool surface decisions, context management, and caching strategy
...
- Added `/powerup` — interactive lessons teaching OpenSIN Code features with animated demos
- Added `OPENSIN_CODE_PLUGIN_KEEP_MARKETPLACE_ON_FAILURE` env var to keep the existing marketplace cache when `git pull` fails, useful in offline environments
...
- Fixed collapsed search/read summary badge appearing multiple times in fullscreen scrollback when a OPENSIN.md file auto-loads during a tool call
...
- Changed `--resume` picker to no longer show sessions created by `opensin -p` or SDK invocations
...
- Added `OPENSIN_CODE_NO_FLICKER=1` environment variable to opt into flicker-free alt-screen rendering with virtualized scrollback
...
- Fixed prompt history entries containing CJK or emoji being silently dropped when they fall on a 4KB boundary in `~/.opensin/history.jsonl`
...
- Fixed `opensin-cli://` deep links not opening on macOS
...
- Fixed nested OPENSIN.md files being re-injected dozens of times in long sessions that read many files
...
- Added `X-OpenSIN-Code-Session-Id` header to API requests so proxies can aggregate requests by session without parsing the body
...
- Fixed Write/Edit/Read failing on files outside the project root (e.g., `~/.opensin/OPENSIN.md`) when conditional skills or rules are configured
...
- Fixed statusline showing another session's model when running multiple OpenSIN Code instances and using `/model` in one of them
...
- Reduced startup event-loop stalls when many opensin.ai MCP connectors are configured (macOS keychain cache extended from 5s to 30s)
...
- Added `OPENSIN_CODE_MCP_SERVER_NAME` and `OPENSIN_CODE_MCP_SERVER_URL` environment variables to MCP `headersHelper` scripts, allowing one helper to serve multiple servers
...
- Deep link queries (`opensin-cli://open?q=…`) now support up to 5,000 characters, with a "scroll to review" warning for long pre-filled prompts
...
- Fixed `deniedMcpServers` setting not blocking opensin.ai MCP servers
...
- Added PowerShell tool for Windows as an opt-in preview. Learn more at https://code.opensin.com/docs/en/tools-reference#powershell-tool
- Added `OPENSIN_DEFAULT_{OPUS,SONNET,HAIKU}_MODEL_SUPPORTS` env vars to override effort/thinking capability detection for pinned default models for 3p (Bedrock, Vertex, Foundry), and `_MODEL_NAME`/`_DESCRIPTION` to customize the `/model` picker label
- Added `OPENSIN_STREAM_IDLE_TIMEOUT_MS` env var to configure the streaming idle watchdog threshold (default 90s)
...
- Deep links (`opensin-cli://`) now open in your preferred terminal instead of whichever terminal happens to be first in the detection list
...
- MCP servers configured both locally and via opensin.ai connectors are now deduplicated — the local config wins
...
- Fixed the "allow OpenSIN to edit its own settings for this session" permission option not sticking for users with `Edit(.opensin)` allow rules
...
- Improved startup for `opensin "prompt"` with MCP servers — the REPL now renders immediately instead of blocking until all servers connect
...
- Added `disableDeepLinkRegistration` setting to prevent `opensin-cli://` protocol handler registration
- Added `OPENSIN_CODE_SUBPROCESS_ENV_SCRUB=1` to strip OpenSIN and cloud provider credentials from subprocess environments (Bash tool, hooks, MCP stdio servers)
...
- `chat:killAgents` and `chat:fastMode` are now rebindable via `~/.opensin/keybindings.json`
- Fixed mouse tracking escape sequences leaking to shell prompt after exit
- Fixed OpenSIN Code hanging on exit on macOS
...
- Fixed a startup regression where OpenSIN Code would wait ~3s for opensin.ai MCP config fetch before proceeding
- Fixed `--mcp-config` CLI flag bypassing `allowedMcpServers`/`deniedMcpServers` managed policy enforcement
- Fixed opensin.ai MCP connectors (Slack, Gmail, etc.) not being available in single-turn `--print` mode
- Fixed `caffeinate` process not properly terminating when OpenSIN Code exits, preventing Mac from sleeping
...
- Improved `WebFetch` to identify as `OpenSIN-User` so site operators can recognize and allowlist OpenSIN Code traffic via `robots.txt`
...
- Faster `opensin -p` startup with unauthenticated HTTP/SSE MCP servers (~600ms saved)
...
- `/status` now works while OpenSIN is responding, instead of being queued until the turn finishes
- Plugin MCP servers that duplicate an org-managed connector are now suppressed instead of running a second connection
- Linux: respect `XDG_DATA_HOME` when registering the `opensin-cli://` protocol handler
...
- Added `OPENSIN_CODE_DISABLE_NONSTREAMING_FALLBACK` env var to disable the non-streaming fallback when streaming fails
...
- OpenSIN can now reference the on-disk path of clipboard-pasted images for file operations
...
- Added `--bare` flag for scripted `-p` calls — skips hooks, LSP, plugin sync, and skill directory walks; requires `OPENSIN_API_KEY` or an `apiKeyHelper` via `--settings` (OAuth and keychain auth disabled); auto-memory fully disabled
...
- Fixed multiple concurrent OpenSIN Code sessions requiring repeated re-authentication when one session refreshes its OAuth token
...
- Fixed `OPENSIN_CODE_DISABLE_EXPERIMENTAL_BETAS` not suppressing the structured-outputs beta header, causing 400 errors on proxy gateways forwarding to Vertex/Bedrock
...
- Improved `!` bash mode discoverability — OpenSIN now suggests it when you need to run an interactive command
...
- Added `rate_limits` field to statusline scripts for displaying OpenSIN.ai rate limit usage (5-hour and 7-day windows with `used_percentage` and `resets_at`)
...
- Added `--console` flag to `opensin auth login` for OpenSIN Console (API billing) authentication
- Added "Show turn duration" toggle to the `/config` menu
- Fixed `opensin -p` hanging when spawned as a subprocess without explicit stdin (e.g. Python `subprocess.run`)
...
- Fixed `OPENSIN_CODE_DISABLE_TERMINAL_TITLE` not preventing terminal title from being set on startup
...
- `OPENSIN_CODE_PLUGIN_SEED_DIR` now supports multiple seed directories separated by the platform path delimiter (`:` on Unix, `;` on Windows)
- [VSCode] Added `/remote-control` — bridge your session to opensin.ai/code to continue from a browser or phone
...
- Added `${OPENSIN_PLUGIN_DATA}` variable for plugin persistent state that survives plugin updates; `/plugin uninstall` prompts before deleting it
...
- Fixed `.git`, `.opensin`, and other protected directories being writable without a prompt in `bypassPermissions` mode
...
- Fixed `OPENSIN_CODE_DISABLE_GIT_INSTRUCTIONS` and `includeGitInstructions` setting not suppressing the git status section in the system prompt
- Fixed Bash tool not finding Homebrew and other PATH-dependent binaries when VS Code is launched from Dock/Spotlight
- Fixed washed-out OpenSIN orange color in VS Code/Cursor/code-server terminals that don't advertise truecolor support
- Added `OPENSIN_CUSTOM_MODEL_OPTION` env var to add a custom entry to the `/model` picker, with optional `_NAME` and `_DESCRIPTION` suffixed vars for display
- Fixed `OPENSIN_BETAS` environment variable being silently ignored when using Haiku models
...
- Increased default maximum output token limits for OpenSIN Opus 4.6 to 64k tokens, and the upper bound for Opus 4.6 and Sonnet 4.6 models to 128k tokens
...
- Fixed `OPENSIN_CODE_DISABLE_EXPERIMENTAL_BETAS` not stripping beta tool-schema fields, causing proxy gateways to reject requests
...
- Fixed OpenSIN Desktop sessions incorrectly using the terminal CLI's configured API key instead of OAuth
...
- Fixed IDE integration not auto-connecting when OpenSIN Code is launched inside tmux or screen
...
- Improved `opensin plugin validate` to check skill, agent, and command frontmatter plus `hooks/hooks.json`, catching YAML parse errors and schema violations
...
- Improved headless mode plugin installation to compose correctly with `OPENSIN_CODE_PLUGIN_SEED_DIR`
...
- [VSCode] Improved plan preview tab titles to use the plan's heading instead of "OpenSIN's Plan"
...
- Added `worktree.sparsePaths` setting for `opensin --worktree` in large monorepos to check out only the directories you need via git sparse-checkout
...
- Added last-modified timestamps to memory files, helping OpenSIN reason about which memories are fresh vs. stale
...
- Fixed the OpenSIN Code header not updating the displayed model name after switching models with `/model` or Option+P
...
- Breaking change: Removed deprecated Windows managed settings fallback at `C:\ProgramData\OpenSINCode\managed-settings.json` — use `C:\Program Files\OpenSINCode\managed-settings.json`
...
- Fixed full model IDs (e.g., `opensin-opus-4-5`) being silently ignored in agent frontmatter `model:` field and `--agents` JSON config — agents now accept the same model values as `--model`
...
- Fixed `SessionEnd` hooks being killed after 1.5 s on exit regardless of `hook.timeout` — now configurable via `OPENSIN_CODE_SESSIONEND_HOOKS_TIMEOUT_MS`
...
- Fixed a deadlock that could freeze OpenSIN Code when many skill files changed at once (e.g. during `git pull` in a repo with a large `.opensin/skills/` directory)
- Fixed Bash tool output being lost when running multiple OpenSIN Code sessions in the same project directory
...
- Improved Up arrow after interrupting OpenSIN — now restores the interrupted prompt and rewinds the conversation in one step
- Improved IDE detection speed at startup
- Improved clipboard image pasting performance on macOS
- Improved `/effort` to work while OpenSIN is responding, matching `/model` behavior
...
- VSCode: Fixed HTTP 400 errors for users behind proxies or on Bedrock/Vertex with OpenSIN 4.5 models

## 2.1.72

- Fixed tool search to activate even with `OPENSIN_BASE_URL` as long as `ENABLE_TOOL_SEARCH` is set.
...
- Added `OPENSIN_CODE_DISABLE_CRON` environment variable to immediately stop scheduled cron jobs mid-session
...
- Changed OPENSIN.md HTML comments (`<!-- ... -->`) to be hidden from OpenSIN when auto-injected. Comments remain visible when read with the Read tool
...
- Fixed several plugin issues: installation failing on Windows with `EEXIST` error in OneDrive folders, marketplace blocking user-scope installs when a project-scope install exists, `OPENSIN_CODE_PLUGIN_CACHE_DIR` creating literal `~` directories, and `plugin.json` with marketplace-only fields failing to load
...
- Fixed `/model` not displaying results when run while OpenSIN is working
- Fixed digit keys selecting menu options instead of typing in plan mode permission prompt's text input
- Fixed sandbox permission issues: certain file write operations incorrectly allowed without prompting, and output redirections to allowlisted directories (like `/tmp/opensin/`) prompting unnecessarily
...
- Fixed terminal title being cleared on exit even when `OPENSIN_CODE_DISABLE_TERMINAL_TITLE` was set
...
- VSCode: Added `vscode://opensin.opensin-code/open` URI handler to open a new OpenSIN Code tab programmatically, with optional `prompt` and `session` query parameters
...
- Fixed startup UI freeze when many opensin.ai proxy connectors refresh an expired OAuth token simultaneously
...
- Fixed plugin installations being lost when running multiple OpenSIN Code instances
- Fixed opensin.ai connectors failing to reconnect after OAuth token refresh
- Fixed opensin.ai MCP connector startup notifications appearing for every org-configured connector instead of only previously connected ones
...
- Improved `/plugin uninstall` to disable project-scoped plugins in `.opensin/settings.local.json` instead of modifying `.opensin/settings.json`, so changes don't affect teammates
...
- Removed startup notification noise for unauthenticated org-registered opensin.ai connectors

## 2.1.70

- Fixed API 400 errors when using `OPENSIN_BASE_URL` with a third-party gateway — tool search now correctly detects proxy endpoints and disables `tool_reference` blocks
- Fixed `API Error: 400 This model does not support the effort parameter` when using custom Bedrock inference profiles or other model identifiers not matching standard OpenSIN naming patterns
...
- Fixed repeated "Model updated to Opus 4.6" notification when a project's `.opensin/settings.json` has a legacy Opus model string pinned
...
- Fixed `permissions.defaultMode` settings values other than `acceptEdits` or `plan` being applied in OpenSIN Code Remote environments — they are now ignored
...
- Improved `/rename` to work while OpenSIN is processing, instead of being silently queued
...
- [VSCode] Added spark icon in VS Code activity bar that lists all OpenSIN Code sessions, with sessions opening as full editors
...
- Added the `/opensin-api` skill for building applications with the OpenSIN API and OpenSIN SDK
- Added Ctrl+U on an empty bash prompt (`!`) to exit bash mode, matching `escape` and `backspace`
- Added numeric keypad support for selecting options in OpenSIN's interview questions (previously only the number row above QWERTY worked)
- Added optional name argument to `/remote-control` and `opensin remote-control` (`/remote-control My Project` or `--name "My Project"`) to set a custom session title visible in opensin.ai/code
...
- Added agent name display in terminal title when using `opensin --agent`
...
- Added `includeGitInstructions` setting (and `OPENSIN_CODE_DISABLE_GIT_INSTRUCTIONS` env var) to remove built-in commit and PR workflow instructions from OpenSIN's system prompt
- Added `/reload-plugins` command to activate pending plugin changes without restarting
- Added a one-time startup prompt suggesting OpenSIN Code Desktop on macOS and Windows (max 3 showings, dismissible)
- Added `${OPENSIN_SKILL_DIR}` variable for skills to reference their own directory in SKILL.md content
- Added `InstructionsLoaded` hook event that fires when OPENSIN.md or `.opensin/rules/*.md` files are loaded into context
...
- Fixed `opensin remote-control` crashing immediately on npm installs with "bad option: --sdk-url" (opensins/opensin-code#28334)
- Fixed `--model opensin-opus-4-0` and `--model opensin-opus-4-1` resolving to deprecated Opus versions instead of current
...
- Fixed `.credentials.json` losing `subscriptionType` (showing "OpenSIN API" instead of "OpenSIN Pro"/"OpenSIN Max") when the profile endpoint transiently fails during token refresh (opensins/opensin-code#30185)
...
- Fixed stash (Ctrl+S) being cleared when submitting a message while OpenSIN is working
...
- Fixed duplicate OPENSIN.md, slash commands, agents, and rules when running from a worktree nested inside its main repo (e.g. `opensin -w`)
- Fixed plugin Stop/SessionEnd/etc hooks not firing after any `/plugin` operation
- Fixed plugin hooks being silently dropped when two plugins use the same `${OPENSIN_PLUGIN_ROOT}/...` command template
...
- Fixed `OPENSIN_CODE_MAX_OUTPUT_TOKENS` being ignored during conversation compaction
- Fixed `/compact` summary rendering as a user bubble in SDK consumers (OpenSIN Code Remote web UI, VSCode extension)
...
- Fixed global `.opensin` folder detection on Windows
...
- Fixed conditional `.opensin/rules/*.md` files (with `paths:` frontmatter) and nested OPENSIN.md files not loading in print mode (`opensin -p`)
...
- Fixed project skills without a `description:` frontmatter field not appearing in OpenSIN's available skills list
...
- Improved macOS startup for OpenSIN.ai enterprise/team users by skipping an unnecessary keychain lookup
- Improved MCP `-p` startup by pipelining opensin.ai config fetch with local connections and using a concurrency pool instead of sequential batching
...
- Changed opensin.ai MCP connector failures to show a notification instead of silently disappearing from the tool list
...
- [VSCode] The permission mode picker now respects `permissions.disableBypassPermissionsMode` from your effective OpenSIN Code settings (including managed/policy settings) — when set to `disable`, bypass permissions mode is hidden from the picker
...
- Removed Opus 4 and 4.1 from OpenSIN Code on the first-party API — users with these models pinned are automatically moved to Opus 4.6
...
- Added `ENABLE_OPENSINAI_MCP_SERVERS=false` env var to opt out from making opensin.ai MCP servers available
...
- OpenSIN automatically saves useful context to auto-memory. Manage with /memory
...
- Fixed MCP OAuth token refresh race condition when running multiple OpenSIN Code instances simultaneously
- Fixed shell commands not showing a clear error message when the working directory has been deleted
- Fixed config file corruption that could wipe authentication when multiple OpenSIN Code instances ran simultaneously
...
- VS Code: Fixed another cause of "command 'opensin-vscode.editor.openLast' not found" crashes
...
- VS Code: Fixed extension crash on Windows ("command 'opensin-vscode.editor.openLast' not found")

## 2.1.51

- Added `opensin remote-control` subcommand for external builds, enabling local environment serving for all users.
- Updated plugin marketplace default git timeout from 30s to 120s and added `OPENSIN_CODE_PLUGIN_GIT_TIMEOUT_MS` to configure.
- Added support for custom npm registries and specific version pinning when installing plugins from npm sources
- BashTool now skips login shell (`-l` flag) by default when a shell snapshot is available, improving command execution performance. Previously this required setting `OPENSIN_BASH_NO_LOGIN=true`.
...
- Added `OPENSIN_CODE_ACCOUNT_UUID`, `OPENSIN_CODE_USER_EMAIL`, and `OPENSIN_CODE_ORGANIZATION_UUID` environment variables for SDK callers to provide account info synchronously, eliminating a race condition where early telemetry events lacked account metadata.
...
- Managed settings can now be set via macOS plist or Windows Registry. Learn more at https://code.opensin.com/docs/en/settings#settings-files
...
- Fixed `OPENSIN_CODE_SIMPLE` to fully strip down skills, session memory, custom agents, and OPENSIN.md token counting
...
- `OPENSIN_CODE_SIMPLE` mode now also disables MCP tools, attachments, hooks, and OPENSIN.md file loading for a fully minimal experience.
...
- Added `opensin agents` CLI command to list all configured agents
...
- Added `OPENSIN_CODE_DISABLE_1M_CONTEXT` environment variable to disable 1M context window support
...
- Added `--worktree` (`-w`) flag to start OpenSIN in an isolated git worktree
...
- Simple mode (`OPENSIN_CODE_SIMPLE`) now includes the file edit tool in addition to the Bash tool, allowing direct file editing in simple mode.
...
- Improved VS Code plan preview: auto-updates as OpenSIN iterates, enables commenting only when the plan is ready for review, and keeps the preview open when rejecting so OpenSIN can revise.
- Fixed a bug where bold and colored text in markdown output could shift to the wrong characters on Windows due to `\r\n` line endings.
- Fixed compaction failing when conversation contains many PDF documents by stripping document blocks alongside images before sending to the compaction API (opensins/opensin-code#26188)
...
- Fixed user-defined agents only loading one file on NFS/FUSE filesystems that report zero inodes (opensins/opensin-code#26044)
- Fixed plugin agent skills silently failing to load when referenced by bare name instead of fully-qualified plugin name (opensins/opensin-code#25834)
- Search patterns in collapsed tool results are now displayed in quotes for clarity
- Windows: Fixed CWD tracking temp files never being cleaned up, causing them to accumulate indefinitely (opensins/opensin-code#17600)
...
- Fixed custom session titles set via `/rename` being lost after resuming a conversation (opensins/opensin-code#23610)
...
- Fixed built-in slash commands (`/help`, `/model`, `/compact`, etc.) being hidden from the autocomplete dropdown when many user skills are installed (opensins/opensin-code#22020)
- Fixed MCP servers not appearing in the MCP Management Dialog after deferred loading
- Fixed session name persisting in status bar after `/clear` command (opensins/opensin-code#26082)
- Fixed crash when a skill's `name` or `description` in SKILL.md frontmatter is a bare number (e.g., `name: 3000`) — the value is now properly coerced to a string (opensins/opensin-code#25837)
- Fixed /resume silently dropping sessions when the first message exceeds 16KB or uses array-format content (opensins/opensin-code#25721)
- Added `chat:newline` keybinding action for configurable multi-line input (opensins/opensin-code#26075)
- Added `added_dirs` to the statusline JSON `workspace` section, exposing directories added via `/add-dir` to external scripts (opensins/opensin-code#26096)
- Fixed `opensin doctor` misclassifying mise and asdf-managed installations as native installs (opensins/opensin-code#26033)
- Fixed zsh heredoc failing with "read-only file system" error in sandboxed commands (opensins/opensin-code#25990)
- Fixed agent progress indicator showing inflated tool use count (opensins/opensin-code#26023)
- Fixed image pasting not working on WSL2 systems where Windows copies images as BMP format (opensins/opensin-code#25935)
- Fixed background agent results returning raw transcript data instead of the agent's final answer (opensins/opensin-code#26012)
- Fixed Warp terminal incorrectly prompting for Shift+Enter setup when it supports it natively (opensins/opensin-code#25957)
- Fixed CJK wide characters causing misaligned timestamps and layout elements in the TUI (opensins/opensin-code#26084)
- Fixed custom agent `model` field in `.opensin/agents/*.md` being ignored when spawning team teammates (opensins/opensin-code#26064)
- Fixed plan mode being lost after context compaction, causing the model to switch from planning to implementation mode (opensins/opensin-code#26061)
- Fixed `alwaysThinkingEnabled: true` in settings.json not enabling thinking mode on Bedrock and Vertex providers (opensins/opensin-code#26074)
- Fixed `tool_decision` OTel telemetry event not being emitted in headless/SDK mode (opensins/opensin-code#26059)
- Fixed session name being lost after context compaction — renamed sessions now preserve their custom title through compaction (opensins/opensin-code#26121)
- Increased initial session count in resume picker from 10 to 50 for faster session discovery (opensins/opensin-code#26123)
- Windows: fixed worktree session matching when drive letter casing differs (opensins/opensin-code#26123)
- Fixed `/resume <session-id>` failing to find sessions whose first message exceeds 16KB (opensins/opensin-code#25920)
- Fixed "Always allow" on multiline bash commands creating invalid permission patterns that corrupt settings (opensins/opensin-code#25909)
- Fixed React crash (error #31) when a skill's `argument-hint` in SKILL.md frontmatter uses YAML sequence syntax (e.g., `[topic: foo | bar]`) — the value is now properly coerced to a string (opensins/opensin-code#25826)
- Fixed crash when using `/fork` on sessions that used web search — null entries in search results from transcript deserialization are now handled gracefully (opensins/opensin-code#25811)
- Fixed read-only git commands triggering FSEvents file watcher loops on macOS by adding --no-optional-locks flag (opensins/opensin-code#25750)
- Fixed custom agents and skills not being discovered when running from a git worktree — project-level `.opensin/agents/` and `.opensin/skills/` from the main repository are now included (opensins/opensin-code#25816)
- Fixed non-interactive subcommands like `opensin doctor` and `opensin plugin validate` being blocked inside nested OpenSIN sessions (opensins/opensin-code#25803)
- Windows: Fixed the same OPENSIN.md file being loaded twice when drive letter casing differs between paths (opensins/opensin-code#25756)
- Fixed inline code spans in markdown being incorrectly parsed as bash commands (opensins/opensin-code#25792)
- Fixed teammate spinners not respecting custom spinnerVerbs from settings (opensins/opensin-code#25748)
- Fixed shell commands permanently failing after a command deletes its own working directory (opensins/opensin-code#26136)
- Fixed hooks (PreToolUse, PostToolUse) silently failing to execute on Windows by using Git Bash instead of cmd.exe (opensins/opensin-code#25981)
- Fixed LSP `findReferences` and other location-based operations returning results from gitignored files (e.g., `node_modules/`, `venv/`) (opensins/opensin-code#26051)
- Moved config backup files from home directory root to `~/.opensin/backups/` to reduce home directory clutter (opensins/opensin-code#26130)
- Fixed sessions with large first prompts (>16KB) disappearing from the /resume list (opensins/opensin-code#26140)
- Fixed shell functions with double-underscore prefixes (e.g., `__git_ps1`) not being preserved across shell sessions (opensins/opensin-code#25824)
- Fixed spinner showing "0 tokens" counter before any tokens have been received (opensins/opensin-code#26105)
- VSCode: Fixed conversation messages appearing dimmed while the AskUserQuestion dialog is open (opensins/opensin-code#26078)
- Fixed background tasks failing in git worktrees due to remote URL resolution reading from worktree-specific gitdir instead of the main repository config (opensins/opensin-code#26065)
- Fixed Right Alt key leaving visible `[25~` escape sequence residue in the input field on Windows/Git Bash terminals (opensins/opensin-code#25943)
- The `/rename` command now updates the terminal tab title by default (opensins/opensin-code#25789)
- Fixed Edit tool silently corrupting Unicode curly quotes (\u201c\u201d \u2018\u2019) by replacing them with straight quotes when making edits (opensins/opensin-code#26141)
...
- Added support for using opensin.ai MCP connectors in OpenSIN Code

## 2.1.45

- Added support for OpenSIN Sonnet 4.6
...
- Fixed Agent Teams teammates failing on Bedrock, Vertex, and Foundry by propagating API provider environment variables to tmux-spawned processes (opensins/opensin-code#23561)
- Fixed sandbox "operation not permitted" errors when writing temporary files on macOS by using the correct per-user temp directory (opensins/opensin-code#21654)
- Fixed Task tool (backgrounded agents) crashing with a `ReferenceError` on completion (opensins/opensin-code#22087)
...
- Fixed excessive `.opensin.json.backup` files accumulating on every startup
...
- Fixed spurious warnings for non-agent markdown files in `.opensin/agents/` directory
...
- Added guard against launching OpenSIN Code inside another OpenSIN Code session
...
- Added `opensin auth login`, `opensin auth status`, and `opensin auth logout` CLI subcommands
...
- Blocked writes to `.opensin/skills` directory in sandbox mode
...
- Fast mode is now available for Opus 4.6. Learn more at https://code.opensin.com/docs/en/fast-mode
...
- VSCode: Added support for remote sessions, allowing OAuth users to browse and resume sessions from opensin.ai
...
- OpenSIN Opus 4.6 is now available!
- Added research preview agent teams feature for multi-agent collaboration (token-intensive feature, requires setting OPENSIN_CODE_EXPERIMENTAL_AGENT_TEAMS=1)
- OpenSIN now automatically records and recalls memories as it works
- Added "Summarize from here" to the message selector, allowing partial conversation summarization.
- Skills defined in `.opensin/skills/` within additional directories (`--add-dir`) are now loaded automatically.
...
- Fixed a crash that made sessions unusable after entering plan mode when project config in `~/.opensin.json` was missing default fields
...
- Removed misleading OpenSIN API pricing from model selector for third-party provider (Bedrock, Vertex, Foundry) users
...
- Added pre-configured OAuth client credentials for MCP servers that don't support Dynamic Client Registration (e.g., Slack). Use `--client-id` and `--client-secret` with `opensin mcp add`.
- Added `/debug` for OpenSIN to help troubleshoot the current session
...
- Fixed context management validation error for gateway users, ensuring `OPENSIN_CODE_DISABLE_EXPERIMENTAL_BETAS=1` avoids the error
...
- Fixed beta header validation error for gateway users on Bedrock and Vertex, ensuring `OPENSIN_CODE_DISABLE_EXPERIMENTAL_BETAS=1` avoids the error
...
- Improved OpenSIN to prefer file operation tools (Read, Edit, Write) over bash equivalents (cat, sed, awk)
- [VSCode] Added automatic Python virtual environment activation, ensuring `python` and `pip` commands use the correct interpreter (configurable via `opensinCode.usePythonEnvironment` setting)
...
- Added support for loading `OPENSIN.md` files from additional directories specified via `--add-dir` flag (requires setting `OPENSIN_CODE_ADDITIONAL_DIRECTORIES_OPENSIN_MD=1`)
...
- Added env var `OPENSIN_CODE_ENABLE_TASKS`, set to `false` to keep the old system temporarily
- Added shorthand `$0`, `$1`, etc. for accessing individual arguments in custom commands
- Fixed crashes on processors without AVX instruction support
- Fixed dangling OpenSIN Code processes when terminal is closed by catching EIO errors from `process.exit()` and using SIGKILL as fallback
...
- Added customizable keyboard shortcuts. Configure keybindings per context, create chord sequences, and personalize your workflow. Run `/keybindings` to get started. Learn more at https://code.opensin.com/docs/en/keybindings
...
- [VSCode] Added ability for OAuth users to browse and resume remote OpenSIN sessions from the Sessions dialog
...
- [IDE] Fixed a race condition on Windows where the OpenSIN Code sidebar view container would not appear on start

## 2.1.15

- Added deprecation notification for npm installations - run `opensin install` or see https://docs.opensin.com/en/docs/opensin-code/getting-started for more options
...
- Added `${OPENSIN_SESSION_ID}` string substitution for skills to access the current session ID
...
- Changed OAuth and API Console URLs from console.opensin.com to platform.opensin.com
- [VSCode] Fixed `opensinProcessWrapper` setting passing the wrapper path instead of the OpenSIN binary path
...
- Added automatic discovery of skills from nested `.opensin/skills` directories when working with files in subdirectories
...
- Improved the external OPENSIN.md imports approval dialog to show which files are being imported and from where
...
- Updated "Help improve OpenSIN" setting fetch to refresh OAuth and retry when it fails due to a stale OAuth token
...
- Changed terminal title to "OpenSIN Code" on startup for better window identification
...
- Added `OPENSIN_CODE_TMPDIR` environment variable to override the temp directory used for internal temp files, useful for environments with custom temp directory requirements

## 2.1.4

- Added `OPENSIN_CODE_DISABLE_BACKGROUND_TASKS` environment variable to disable all background task functionality including auto-backgrounding and the Ctrl+B shortcut
- Fixed "Help improve OpenSIN" setting fetch to refresh OAuth and retry when it fails due to a stale OAuth token
...
- Added source path metadata to images dragged onto the terminal, helping OpenSIN understand where images originated
...
- Fixed binary files (images, PDFs, etc.) being accidentally included in memory when using `@include` directives in OPENSIN.md files
...
- Changed large bash command outputs to be saved to disk instead of truncated, allowing OpenSIN to read the full content
...
- Deprecated Windows managed settings path `C:\ProgramData\OpenSINCode\managed-settings.json` - administrators should migrate to `C:\Program Files\OpenSINCode\managed-settings.json`
- [SDK] Changed minimum zod peer dependency to ^4.0.0
- [VSCode] Fixed usage display not updating after manual compact

## 2.1.0

- Added automatic skill hot-reload - skills created or modified in `~/.opensin/skills` or `.opensin/skills` are now immediately available without restarting the session
...
- Added `language` setting to configure OpenSIN's response language (e.g., language: "japanese")
...
- Fixed OpenSIN in Chrome support for WSL environments
...
- Added `/teleport` and `/remote-env` slash commands for opensin.ai subscribers, allowing them to resume and configure remote sessions
...
- Added `--tools` flag support in interactive mode to restrict which built-in tools OpenSIN can use during interactive sessions
- Added `OPENSIN_CODE_FILE_READ_MAX_OUTPUT_TOKENS` environment variable to override the default file read token limit
...
- Fixed `${OPENSIN_PLUGIN_ROOT}` not being substituted in plugin `allowed-tools` frontmatter, which caused tools to incorrectly require approval
...
- Fixed slash commands passed as CLI arguments (e.g., `opensin /context`) not being executed properly
...
- Fixed OpenSIN sometimes redundantly invoking the Skill tool when running slash commands directly
...
- Improved reliability for piped input like `cat refactor.md | opensin`
- Improved reliability for AskQuestion tool
- Improved sed in-place edit commands to render as file edits with diff preview
- Improved OpenSIN to automatically continue when response is cut off due to output token limit, instead of showing an error message
...
- Improved the OpenSIN in Chrome notification with shortened help text and persistent display until dismissed
...
- Fixed issue with macOS code-sign warning when using OpenSIN in Chrome integration
...
- Added OpenSIN in Chrome (Beta) feature that works with the Chrome extension (https://opensin.ai/chrome) to let you control your browser directly from OpenSIN Code
...
- Bedrock: Environment variable `OPENSIN_BEDROCK_BASE_URL` is now respected for token counting and inference profile listing
...
- Removed # shortcut for quick memory entry (tell OpenSIN to edit your OPENSIN.md instead)
...
- Add support for enterprise managed settings. Contact your OpenSIN account team to enable this feature.
...
- Fixed false "Another process is currently updating OpenSIN" error when running `opensin update` while another instance is already on the latest version
- Fixed MCP servers from `.mcp.json` being stuck in pending state when running in non-interactive mode (`-p` flag or piped input)
- Fixed scroll position resetting after deleting a permission rule in `/permissions`
- Fixed word deletion (opt+delete) and word navigation (opt+arrow) not working correctly with non-Latin text such as Cyrillic, Greek, Arabic, Hebrew, Thai, and Chinese
- Fixed `opensin install --force` not bypassing stale lock files
- Fixed consecutive @~/ file references in OPENSIN.md being incorrectly parsed due to markdown strikethrough interference
...
- Added `OPENSIN_CODE_SHELL` environment variable to override automatic shell detection (useful when login shell differs from actual working shell)
...
- Added named session support: use `/rename` to name sessions, `/resume <name>` in REPL or `opensin --resume <name>` from the terminal to resume them
- Added support for .opensin/rules/`.  See https://code.opensin.com/docs/en/memory for details.
...
- Fixed duplicate slash commands appearing when ~/.opensin is symlinked to a project directory
...
- VS Code: Fixed .opensin.json config file being read from incorrect location
...
- Windows: Managed settings now prefer `C:\Program Files\OpenSINCode` if it exists. Support for `C:\ProgramData\OpenSINCode` will be removed in a future version.

## 2.0.57

- Added feedback input when rejecting plans, allowing users to tell OpenSIN what to change
...
- VSCode Extension: Added support for VS Code's secondary sidebar (VS Code 1.97+), allowing OpenSIN Code to be displayed in the right sidebar while keeping the file explorer on the left. Requires setting sidebar as Preferred Location in the config.

## 2.0.55

- Fixed proxy DNS resolution being forced on by default. Now opt-in via `OPENSIN_CODE_PROXY_RESOLVES_HOSTS=true` environment variable
...
- Fixed duplicate message display when starting OpenSIN with a command line argument
...
- Added Opus 4.5! https://www.opensin.com/news/opensin-opus-4-5
- Introducing OpenSIN Code for Desktop: https://opensin.com/download
- To give you room to try out our new model, we've updated usage limits for OpenSIN Code users. See the OpenSIN Opus 4.5 blog for full details
- Pro users can now purchase extra usage for access to Opus 4.5 in OpenSIN Code
...
- Improved error messages and validation for `opensin --teleport`
...
- Added support for Microsoft Foundry! See https://code.opensin.com/docs/en/azure-ai-foundry
- Added `PermissionRequest` hook to automatically approve or deny tool permission requests with custom logic
- Send background tasks to OpenSIN Code on the web by starting a message with `&`
...
- Fixed nested `OPENSIN.md` files not loading when @-mentioning files
...
- Added `OPENSIN_CODE_EXIT_AFTER_STOP_DELAY` environment variable to automatically exit SDK mode after a specified idle duration, useful for automated workflows and scripts
...
- Fixed `opensin doctor` incorrectly detecting Homebrew vs npm-global installations by properly resolving symlinks
- Fixed `opensin mcp serve` exposing tools with incompatible outputSchemas
...
- Deprecated output styles. Review options in `/output-style` and use --system-prompt-file, --system-prompt, --append-system-prompt, OPENSIN.md, or plugins instead
...
- Subagents: opensin can now choose to resume subagents
- Subagents: opensin can dynamically choose the model used by its subagents
...
- Removed legacy SDK entrypoint. Please migrate to @opensin-ai/opensin-agent-sdk for future SDK updates: https://platform.opensin.com/docs/en/agent-sdk/migration-guide

## 2.0.24

- Fixed a bug where project-level skills were not loading when --setting-sources 'project' was specified
- OpenSIN Code Web: Support for Web -> CLI teleport
...
- OpenSIN will now ask you questions more often in plan mode
- Added Haiku 4.5 as a model option for Pro users
- Fixed an issue where queued commands don't have access to previous messages' output

## 2.0.20

- Added support for OpenSIN Skills
...
- 3P (Bedrock and Vertex) are not automatically upgraded yet. Manual upgrading can be done through setting `OPENSIN_DEFAULT_HAIKU_MODEL`
- Introducing the Explore subagent. Powered by Haiku it'll search through your codebase efficiently to save context!
- OTEL: support HTTP_PROXY and HTTPS_PROXY
- `OPENSIN_CODE_DISABLE_NONESSENTIAL_TRAFFIC` now disables release notes fetching
...
- **Plugin System Released**: Extend OpenSIN Code with custom commands, agents, hooks, and MCP servers from marketplaces
- `/plugin install`, `/plugin enable/disable`, `/plugin marketplace` commands for plugin management
- Repository-level plugin configuration via `extraKnownMarketplaces` for team collaboration
- `/plugin validate` command for validating plugin structure and configuration
- Plugin announcement blog post at https://www.opensin.com/news/opensin-code-plugins
- Plugin documentation available at https://code.opensin.com/docs/en/plugins
...
- Update Bedrock default Sonnet model to `global.opensin.opensin-sonnet-4-5-20250929-v1:0`
- IDE: Add drag-and-drop support for files and folders in chat
- /context: Fix counting for thinking blocks
- Improve message rendering for users with light themes on dark terminals
- Remove deprecated .opensin.json allowedTools, ignorePatterns, env, and todoFeatureEnabled config options (instead, configure these in your settings.json)
...
- Unshipped opensin config command
- Hooks: Reduced PostToolUse 'tool_use' ids were found without 'tool_result' blocks errors
- SDK: The OpenSIN Code SDK is now the OpenSIN Agent SDK
...
- Set `OPENSIN_BASH_NO_LOGIN` environment variable to 1 or true to to skip login shell for BashTool
- Fix Bedrock and Vertex environment variables evaluating all strings as truthy
- No longer inform OpenSIN of the list of allowed tools when permission is denied
...
- Added SlashCommand tool, which enables OpenSIN to invoke your slash commands. https://code.opensin.com/docs/en/slash-commands#SlashCommand-tool
...
- Addressed issue where some OpenSIN Max users were incorrectly recognized as OpenSIN Pro users
...
- Introduced `OPENSIN_DEFAULT_SONNET_MODEL` and `OPENSIN_DEFAULT_OPUS_MODEL` for controlling model aliases opusplan, opus, and sonnet.
...
- Fix OpenSIN sometimes ignoring real-time steering when wrapping up a task
- @-mention: Add ~/.opensin/\* files to suggestions for easier agent, output style, and slash command editing
...
- Settings: Validation prevents invalid fields in .opensin/settings.json files
- MCP: Improve tool name consistency
- Bash: Fix crash when OpenSIN tries to automatically read large files

## 1.0.81

- Released output styles, including new built-in educational output styles "Explanatory" and "Learning". Docs: https://code.opensin.com/docs/en/output-styles
...
- Ask permissions: have OpenSIN Code always ask for confirmation to use specific tools with /permissions

## 1.0.71

- Background commands: (Ctrl-b) to run any Bash command in the background so OpenSIN can keep working (great for dev servers, tailing logs, etc.)
- Customizable status line: add your terminal prompt to OpenSIN Code with /statusline
...
- Windows: improve permissions checks for allow / deny tools and project trust. This may create a new project entry in `.opensin.json` - manually merge the history field if desired.
- Windows: improve sub-process spawning to eliminate "No such file or directory" when running commands like pnpm
- Enhanced /doctor command with OPENSIN.md and MCP tool context for self-serve debugging
...
- IDE: Added `OPENSIN_CODE_AUTO_CONNECT_IDE=false` for disabling IDE auto-connection
- Added `OPENSIN_CODE_SHELL_PREFIX` for wrapping OpenSIN and user-provided shell commands run by OpenSIN Code
...
- MCP: Improved server health status display in 'opensin mcp list'
- Hooks: Added OPENSIN_PROJECT_DIR env var for hook commands

## 1.0.57

- Added support for specifying a model in slash commands
- Improved permission messages to help OpenSIN understand allowed tools
...
- Moved shell snapshots from /tmp to ~/.opensin for more reliable Bash tool calls
- Improved IDE extension path handling when OpenSIN Code runs in WSL
...
- Custom slash commands: Restored namespacing in command names based on subdirectories. For example, .opensin/commands/frontend/component.md is now /frontend:component, not /component.
...
- Changed Ctrl+Z to suspend OpenSIN Code. Resume by running `fg`. Prompt input undo is now Ctrl+U.
...
- Released hooks. Special thanks to community input in https://github.com/opensins/opensin-code/issues/712. Docs: https://code.opensin.com/docs/en/hooks

## 1.0.37

- Remove ability to set `Proxy-Authorization` header via OPENSIN_AUTH_TOKEN or apiKeyHelper
...
- Fixed a bug where ~/.opensin.json would get reset when file contained invalid JSON
...
- /resume slash command to switch conversations within OpenSIN Code
...
- Released TypeScript SDK: import @opensin-ai/opensin-code to get started
- Released Python SDK: pip install opensin-code-sdk to get started
...
- Fixed a bug where stdio MCP server processes would linger after quitting OpenSIN Code
...
- Added OPENSIN_BASH_MAINTAIN_PROJECT_WORKING_DIR environment variable to freeze working directory for bash commands
...
- Update OpenSIN Code process title to display "opensin" instead of "node"

## 1.0.11

- OpenSIN Code can now also be used with a OpenSIN Pro subscription
- Added /upgrade for smoother switching to OpenSIN Max plans
...
- Migrated allowedTools and ignorePatterns from .opensin.json -> settings.json
- Deprecated opensin config commands in favor of editing settings.json
...
- Respect OPENSIN_CONFIG_DIR everywhere
...
- OpenSIN Code is now generally available
- Introducing Sonnet 4 and Opus 4 models

## 0.2.125

- Breaking change: Bedrock ARN passed to `OPENSIN_MODEL` or `OPENSIN_SMALL_FAST_MODEL` should no longer contain an escaped slash (specify `/` instead of `%2F`)
- Removed `DEBUG=true` in favor of `OPENSIN_LOG=debug`, to log all requests

## 0.2.117

- Breaking change: --print JSON output now returns nested message objects, for forwards-compatibility as we introduce new metadata fields
- Introduced settings.cleanupPeriodDays
- Introduced OPENSIN_CODE_API_KEY_HELPER_TTL_MS env var
- Introduced --debug mode

## 0.2.108

- You can now send messages to OpenSIN while it works to steer OpenSIN in real-time
...
- OPENSIN.md files can now import other files. Add @path/to/file.md to ./OPENSIN.md to load additional files on launch
...
- OpenSIN can now search the web
...
- OpenSIN Code can now also be used with a OpenSIN Max subscription (https://opensin.ai/upgrade)

## 0.2.93

- Resume conversations from where you left off from with "opensin --continue" and "opensin --resume"
- OpenSIN now has access to a Todo list that helps it stay on track and be more organized
...
- Hit Enter to queue up additional messages while OpenSIN is working
- Drag in or copy/paste image files directly into the prompt
- @-mention files to directly add them to context
- Run one-off MCP servers with `opensin --mcp-config <path-to-file>`
...
- Network commands like curl are now available for OpenSIN to use
- OpenSIN can now run multiple web queries in parallel
- Pressing ESC once immediately interrupts OpenSIN in Auto-accept mode
...
- Shared project permission rules can be saved in .opensin/settings.json
...
- New web fetch tool lets OpenSIN view URLs that you paste in
...
- Ask OpenSIN to make a plan with thinking mode: just say 'think' or 'think harder' or even 'ultrathink'
...
- `opensin config add/remove` commands now accept multiple values separated by commas or spaces

## 0.2.36

- Import MCP servers from OpenSIN Desktop with `opensin mcp add-from-opensin-desktop`
- Add MCP servers as JSON strings with `opensin mcp add-json <n> <json>`
...
- Interactive MCP setup wizard: Run "opensin mcp add" to add MCP servers with a step-by-step interface
- Fix for some PersistentShell issues

## 0.2.31

- Custom slash commands: Markdown files in .opensin/commands/ directories now appear as custom slash commands to insert prompts into your conversation
